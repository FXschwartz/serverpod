...
class ReactiveDatabaseCallEntryRepository {
  ...
  /// Upserts all [ReactiveDatabaseCallEntry]s in the list and returns the resulting rows.
  ///
  ...
  Future<List<ReactiveDatabaseCallEntry>> upsert(
    _i1.DatabaseSession session,
    List<ReactiveDatabaseCallEntry> rows, {
    required _i1.ColumnSelections<ReactiveDatabaseCallEntryTable>
    conflictColumns,
    ...
    return session.db.upsert<ReactiveDatabaseCallEntry>(
      rows,
      conflictColumns: conflictColumns(ReactiveDatabaseCallEntry.t),
  ...
  /// Upserts a single [ReactiveDatabaseCallEntry] and returns the resulting row.
  ///
  ...
  ///
  /// The returned [ReactiveDatabaseCallEntry] will have its `id` field set.
  Future<ReactiveDatabaseCallEntry?> upsertRow(
    _i1.DatabaseSession session,
    ReactiveDatabaseCallEntry row, {
    required _i1.ColumnSelections<ReactiveDatabaseCallEntryTable>
    conflictColumns,
    ...
    return session.db.upsertRow<ReactiveDatabaseCallEntry>(
      row,
      conflictColumns: conflictColumns(ReactiveDatabaseCallEntry.t),
      updateColumns: updateColumns?.call(ReactiveDatabaseCallEntry.t),
      updateWhere: updateWhere?.call(ReactiveDatabaseCallEntry.t),
